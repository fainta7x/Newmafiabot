...
- **Hard per-request PR budget:** for one user message/request, create or actively advance no more than **3 PRs total**. After the third PR, stop repository changes and CI polling, give a compact handoff of what was completed/what remains, and wait for a new user message before continuing the next batch. Do not bypass this limit by splitting work into more stacked PRs inside the same request.
- A PR may receive several coherent commits, but do not turn one PR into an unbounded repair loop. Batch related fixes before the next expensive CI run; if visual/CI follow-up keeps expanding, record the remaining issues and continue them in the next user request instead of extending the current request indefinitely.
