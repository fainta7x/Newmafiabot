...
import { AlertCircle, ArrowRight, Calendar, Check, CheckCircle2, RefreshCw } from 'lucide-react';
    ...
    <div className="mx-auto w-full max-w-2xl space-y-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[13px] text-text-secondary">Ближайший вечер и реальные действия, которые требуют твоего внимания.</p>
        </div>
      ...
      {nextEvening ? (() => {
        ...
        const going = Number(nextEvening.goingCount || 0);
        const later = Number(nextEvening.laterCount || 0);
        const thinking = Number(nextEvening.thinkingCount || 0);
        const declined = Number(nextEvening.declinedCount || 0);
        const unanswered = Number(nextEvening.unansweredCount || 0);
        const audience = Number(nextEvening.audienceCount || 0);
        const responded = Number(nextEvening.respondedCount || 0);
        return (
            ...
            <div className="p-4 sm:p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[12px] font-bold uppercase tracking-[0.12em] text-accent">{formatLabel(nextEvening.format)}</p>
                  <h3 className="mt-1 break-words text-[19px] font-black leading-tight text-text-primary">{nextEvening.title}</h3>
                  <p className="mt-1 text-[13px] text-text-secondary">{when.date} · {when.time}{nextEvening.venue ? ` · ${nextEvening.venue}` : ''}</p>
                </div>
              </div>
              <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2">
                <strong className="text-[15px] text-text-primary">{going} идут</strong>
                <strong className="text-[15px] text-text-primary">{later} позже</strong>
                <strong className="text-[15px] text-text-primary">{thinking} думают</strong>
              </div>
              <p className="mt-2 text-[12px] text-text-secondary">{declined} не идут{unanswered ? ` · ${unanswered} не ответили` : ''}</p>
              {audience > 0 && unanswered > 0 ? <p className="mt-1 text-[11px] text-text-muted">Ответили {responded} из {audience} известных участников опроса</p> : null}
            </div>
            <div className="border-t border-border-soft p-3">
              <button type="button" onClick={() => onOpenEvening(nextEvening.id)} className="inline-flex min-h-[48px] w-full items-center justify-center gap-2 rounded-[13px] bg-accent px-4 text-[13px] font-bold text-white">Открыть вечер <ArrowRight className="h-4 w-4" /></button>
            </div>
          </section>
        );
      })() : (
        <section className="rounded-[20px] border border-border-soft bg-surface-1 p-5 text-center">
          ...
          <p className="mx-auto mt-1 max-w-sm text-[13px] leading-relaxed text-text-secondary">Создай следующее событие — ответы игроков появятся здесь после анонса.</p>
      ...
      <section className="space-y-3">
        <div className="flex items-end justify-between gap-3">
          <div><h3 className="text-[18px] font-black text-text-primary">Сделать сейчас</h3><p className="mt-0.5 text-[12px] text-text-secondary">Только ситуации, где действительно нужно действие организатора.</p></div>
          {queue.length > shownQueue.length ? <button type="button" onClick={() => onNavigateTab('tasks')} className="min-h-[44px] shrink-0 text-[12px] font-bold text-accent">Показать всё</button> : null}
        </div>
        {shownQueue.length === 0 ? (
          <div className="flex min-h-[88px] items-center gap-3 rounded-[18px] border border-border-soft bg-surface-1 px-4 py-3">
            ...
            <div><strong className="block text-[14px] text-text-primary">На сегодня всё готово</strong><span className="mt-0.5 block text-[12px] text-text-secondary">Неответившие остаются состоянием вечера, а не списком задач.</span></div>
          </div>
        ) : (
