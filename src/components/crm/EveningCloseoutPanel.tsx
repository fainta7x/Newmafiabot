...
export const EveningCloseoutPanel: React.FC<{ eveningId: string; onSettled?: () => void }> = ({ eveningId, onSettled }) => {
  ...
  return <section className="rounded-[20px] border border-warning/25 bg-surface-1 p-3 sm:p-4" data-testid="evening-closeout-panel">
    ...
    <details open={!attendanceReady} className="mt-3 rounded-[14px] border border-border-soft bg-surface-2">
      <summary className="flex min-h-11 cursor-pointer items-center gap-2 px-3 text-[14px] font-semibold text-text-primary"><Users className="h-4 w-4 text-accent" /> Явка и пришедшие без записи</summary>
    ...
    <details open={state.games.needs_override} className="mt-2 rounded-[14px] border border-border-soft bg-surface-2">
      ...
      <div className="border-t border-border-soft p-3">{state.games.needs_override ? <label className="flex min-h-11 cursor-pointer items-start gap-2 rounded-xl bg-warning-soft p-2.5"><input type="checkbox" checked={allowMissingStats} onChange={(e) => setAllowMissingStats(e.target.checked)} className="mt-1" /><span className="text-[13px] leading-5 text-text-secondary"><strong className="text-warning">Закрыть без полной игровой статистики.</strong> {state.games.unfinished.length ? `Незавершённых игр: ${state.games.unfinished.length}. В статистику они не попадут.` : 'Игры за этот вечер не внесены.'}</span></label> : <div className="flex items-center gap-2 text-[12px] text-success"><CheckCircle2 className="h-4 w-4" /> Все внесённые игры завершены.</div>}</div>
    </details>
