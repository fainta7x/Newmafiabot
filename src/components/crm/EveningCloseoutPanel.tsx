...
export const EveningCloseoutPanel: React.FC<{ eveningId: string }> = ({ eveningId }) => {
  ...
  const patchParticipants = async (updates: Array<Partial<Participant> & { id: string }>) => {
    ...
    setState((current) => current ? reconcileCloseoutParticipants(
      current,
    ...
    try {
      const result = await request<BulkUpdateResponse>(`/api/evenings/${encodeURIComponent(eveningId)}/participants/bulk`, {
      ...
      setState((current) => current ? reconcileCloseoutParticipants(
        current,
        current.participants.map((participant) => previousById.get(participant.id) || participant),
      ) : current);
  ...
  const addWalkIn = async (input: { player_id?: string; nickname?: string }) => {
    ...
    try {
      const participant = await request<Participant>(`/api/evenings/${encodeURIComponent(eveningId)}/closeout/walk-in`, {
  ...
  return (
    <section className="rounded-[20px] border border-warning/25 bg-surface-1 p-4" data-testid="evening-closeout-panel">
      <div className="flex items-start justify-between gap-3">
        ...
        <button type="button" onClick={() => void load()} disabled={Boolean(busyAction) || busyIds.size > 0} className="rounded-xl bg-surface-2 p-2 text-text-muted disabled:opacity-40"><RefreshCw className={`h-4 w-4 ${busyAction === 'load' ? 'animate-spin' : ''}`} /></button>
      </div>
      ...
      <div className="mt-4 rounded-[14px] bg-surface-2 p-3">
        ...
        {state.pending_expected.length ? <>
          <div className="mt-3 flex gap-2">
            <button type="button" disabled={Boolean(busyAction) || state.pending_expected.some((item) => busyIds.has(item.id))} onClick={() => void patchParticipants(state.pending_expected.map((item) => ({ id: item.id, attendance_status: 'attended' })))} className="min-h-10 flex-1 rounded-xl bg-success-soft px-2 text-[10px] font-bold text-success disabled:opacity-50">Все были</button>
            <button type="button" disabled={Boolean(busyAction) || state.pending_expected.some((item) => busyIds.has(item.id))} onClick={() => void patchParticipants(state.pending_expected.map((item) => ({ id: item.id, attendance_status: 'no_show' })))} className="min-h-10 flex-1 rounded-xl bg-danger-soft px-2 text-[10px] font-bold text-danger disabled:opacity-50">Остальных не было</button>
          </div>
          <div className="mt-2 space-y-1.5">
            {state.pending_expected.map((item) => <div key={item.id} className="flex items-center gap-2 rounded-xl bg-surface-1 px-3 py-2"><span className="min-w-0 flex-1 truncate text-[11px] font-bold text-text-primary">{item.nickname}</span><button type="button" disabled={busyIds.has(item.id)} onClick={() => void patchParticipants([{ id: item.id, attendance_status: 'attended' }])} className="rounded-lg bg-success-soft px-2.5 py-1.5 text-[9px] font-bold text-success disabled:opacity-40">Был</button><button type="button" disabled={busyIds.has(item.id)} onClick={() => void patchParticipants([{ id: item.id, attendance_status: 'no_show' }])} className="rounded-lg bg-danger-soft px-2.5 py-1.5 text-[9px] font-bold text-danger disabled:opacity-40">Не был</button></div>)}
          </div>
        ...
        {showWalkIn ? <div className="mt-2 rounded-xl border border-border-soft bg-surface-1 p-2.5">
          ...
          <div className="mt-2 flex gap-2"><input value={guestNickname} onChange={(e) => setGuestNickname(e.target.value)} placeholder="Или новый гость" className="min-h-9 min-w-0 flex-1 rounded-lg bg-surface-2 px-2 text-[10px] text-text-primary outline-none" /><button disabled={!guestNickname.trim() || Boolean(busyAction)} onClick={() => void addWalkIn({ nickname: guestNickname.trim() })} className="rounded-lg bg-accent px-3 text-[9px] font-bold text-white disabled:opacity-40">Добавить</button></div>
        </div> : null}
      ...
      <div className="mt-3 rounded-[14px] bg-surface-2 p-3">
        ...
        {state.outstanding.length ? <div className="mt-2 space-y-1.5">{state.outstanding.map((item) => {
          ...
          return <div key={item.id} className="flex items-center gap-2 rounded-xl bg-surface-1 px-3 py-2"><div className="min-w-0 flex-1"><div className="truncate text-[10px] font-bold text-text-primary">{item.nickname}</div><div className="text-[9px] text-text-muted">осталось {money(balance)}</div></div><button type="button" aria-label={`Отметить оплату ${item.nickname}`} disabled={busyIds.has(item.id)} onClick={() => void patchParticipants([{ id: item.id, amount_paid: Number(item.amount_due || 0), payment_status: 'paid' }])} className="rounded-lg bg-success-soft px-2.5 py-1.5 text-[9px] font-bold text-success disabled:opacity-50">{busyIds.has(item.id) ? '…' : 'Оплачено'}</button></div>;
      ...
      <button type="button" disabled={Boolean(busyAction) || busyIds.size > 0 || state.pending_expected.length > 0 || (state.games.needs_override && !allowMissingStats)} onClick={() => void settle()} className="mt-4 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-[13px] bg-accent px-3 text-[11px] font-black text-white disabled:opacity-35"><CheckCircle2 className="h-4 w-4" /> {busyAction === 'settle' ? 'Закрываем…' : 'Закрыть вечер'}</button>
