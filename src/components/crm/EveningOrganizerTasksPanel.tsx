...
export default function EveningOrganizerTasksPanel({
  ...
  return <section className="rounded-[20px] border border-border-soft bg-surface-1 p-4">
    <div className="flex items-start justify-between gap-3">
      <div><div className="text-[10px] font-black uppercase tracking-[0.12em] text-accent">Задачи вечера</div><h3 className="mt-0.5 text-[14px] font-black text-text-primary">{heading}</h3><p className="mt-1 text-[10px] leading-4 text-text-muted">Шаблонные и ручные задачи остаются привязаны к этому вечеру, пока ты их не закроешь.</p></div>
      <button type="button" onClick={() => { setManualStage(activeStage); setManualOpen(true); }} className="inline-flex min-h-9 shrink-0 items-center gap-1 rounded-[10px] bg-accent px-2.5 text-[10px] font-black text-white"><Plus className="h-3.5 w-3.5" /> Своя</button>
    </div>

    {error ? <div className="mt-3 rounded-[11px] bg-danger-soft px-3 py-2 text-[10px] text-danger">{error}</div> : null}
    ...
    {usefulTemplates.length ? <div className="mt-3 flex gap-2 overflow-x-auto pb-1 scrollbar-none">
      {usefulTemplates.map((template) => {
        ...
        return <button key={template.id} type="button" disabled={Boolean(busy) || exists} onClick={() => void addTemplate(template.id)} className="min-h-10 min-w-[142px] rounded-[11px] border border-border-soft bg-surface-2 px-3 text-left disabled:opacity-45"><div className="flex items-center gap-1 text-[10px] font-black text-accent"><Sparkles className="h-3 w-3" />{stageLabel[template.stage]}</div><div className="mt-0.5 truncate text-[10px] font-bold text-text-primary">{exists ? '✓ ' : ''}{template.title}</div></button>;
      })}
    </div> : null}

    {tasks.length ? <div className="mt-3 space-y-1.5">{tasks.map((task) => <div key={task.id} className="flex items-center gap-2 rounded-[12px] bg-surface-2 px-2.5 py-2.5"><span className="min-w-0 flex-1"><span className="text-[10px] font-black uppercase tracking-wide text-text-muted">{stageLabel[taskStage(task)]}</span><strong className="mt-0.5 block truncate text-[10px] text-text-primary">{task.title}</strong>{task.description ? <span className="mt-0.5 block truncate text-[10px] text-text-muted">{task.description}</span> : null}</span><button type="button" disabled={Boolean(busy)} onClick={() => void snooze(task)} title="Отложить на 30 минут" className="grid h-8 w-8 shrink-0 place-items-center rounded-[9px] bg-surface-1 text-text-muted disabled:opacity-40"><Clock3 className="h-3.5 w-3.5" /></button><button type="button" disabled={Boolean(busy)} onClick={() => void complete(task)} title="Выполнено" className="grid h-8 w-8 shrink-0 place-items-center rounded-[9px] bg-success-soft text-success disabled:opacity-40"><Check className="h-3.5 w-3.5" /></button></div>)}</div> : <div className="mt-3 rounded-[12px] border border-dashed border-border-soft p-4 text-center"><RotateCcw className="mx-auto h-4 w-4 text-text-muted" /><div className="mt-1 text-[10px] font-bold text-text-primary">Пока нет задач на этот вечер</div><div className="mt-0.5 text-[10px] text-text-muted">Добавь шаблон выше или создай свою.</div></div>}
