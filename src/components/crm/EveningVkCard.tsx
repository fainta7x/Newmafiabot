...
export const EveningVkCard: React.FC<Props> = ({ eveningId, status, readonly }) => {
  ...
  const connectVkApi = async () => {
    if (busy) return;
    setBusy('connect-api'); setError(null); setMessage(null);
    try {
      const returnTo = `${window.location.pathname}${window.location.search}${window.location.hash}`;
      const body = await request('/api/integrations/vk/oauth/legacy/start', {
        method: 'POST',
        body: JSON.stringify({ return_to: returnTo }),
      });
      if (!body?.authorize_url) throw new Error('Сервер не вернул ссылку API VK');
      window.location.assign(String(body.authorize_url));
    } catch (err: any) {
      setError(err?.message || 'Не удалось подключить API VK');
      setBusy(null);
    }
  };
  ...
  return (
    <section className="rounded-[14px] border border-border-soft bg-surface-2 p-3">
      ...
      {state.integration.oauth?.managed_connected && state.integration.oauth.api_compatible === false ? (
        <div className="mt-2 rounded-xl bg-warning-soft px-3 py-2.5 text-[10px] leading-4 text-warning">
          <div>VK ID подключён, но его токен не подходит для редактирования стены и публикации в канал. Подключи API-токен VK один раз.</div>
          {!readonly ? <button type="button" disabled={Boolean(busy)} onClick={() => void connectVkApi()} className="mt-2 inline-flex min-h-9 items-center gap-2 rounded-[10px] bg-surface-1 px-3 py-2 text-[10px] font-bold text-text-primary disabled:opacity-40"><Link2 className="h-3.5 w-3.5" />{busy === 'connect-api' ? 'Открываем VK…' : 'Подключить API VK'}</button> : null}
        </div>
      ) : null}
