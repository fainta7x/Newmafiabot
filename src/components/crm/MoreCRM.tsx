...
type MenuItem = { id: string; label: string; detail: string; icon: React.ComponentType<{ className?: string }>; onClick: () => void; };
...
const menuTone = (id: string) => {
  if (id === 'tasks' || id === 'betting' || id === 'tournaments') return 'border-amber-200/10 bg-amber-200/[0.08] text-amber-100';
...
const MenuRow = ({ id, label, detail, icon: Icon, onClick }: MenuItem) => (
  <button data-testid={`crm-more-${id}`} type="button" onClick={onClick} className="flex min-h-[60px] w-full items-center gap-3 rounded-[16px] px-2.5 text-left active:bg-white/[0.055] sm:min-h-[64px] sm:px-3">
    <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-[14px] border ${menuTone(id)}`}><Icon className="h-[18px] w-[18px]" /></span>
    <span className="min-w-0 flex-1"><strong className="block text-[14px] font-semibold text-white">{label}</strong><span className="mt-0.5 block line-clamp-1 text-[12px] leading-4 text-white/45">{detail}</span></span>
...
export const MoreCRM: React.FC<MoreCRMProps> = ({ onOpenTasks, onOpenAnalytics, onOpenTheme, onOpenGameEngine, evenings = [], onLogout, onOpenPlayerMusic, onOpenPlayer, activeScreen, onScreenChange }) => {
  ...
  const setSubscreen = (screen: Subscreen) => { if (activeScreen === undefined) setLocalSubscreen(screen); onScreenChange?.(screen); };
  ...
  return <div className="mx-auto w-full max-w-xl space-y-4">
    <header className="px-1 pt-0.5"><h2 className="text-[22px] font-semibold text-white">Ещё</h2><p className="mt-1 text-[13px] leading-5 text-white/45">Частые рабочие инструменты сверху. Редкие настройки и обслуживание скрыты отдельно.</p></header>
    <section className="space-y-1.5"><div className="px-1 text-[12px] font-semibold uppercase tracking-[0.12em] text-white/45">В работе</div><div className="rounded-[20px] border border-white/10 bg-white/[0.035] p-1.5">{dailyItems.map((item) => <MenuRow key={item.id} {...item} />)}</div></section>
    <section className="space-y-1.5"><div className="px-1 text-[12px] font-semibold uppercase tracking-[0.12em] text-white/45">Отчёты</div><div className="rounded-[20px] border border-white/10 bg-white/[0.035] p-1.5">{reportItems.map((item) => <MenuRow key={item.id} {...item} />)}</div></section>
    <section className="rounded-[20px] border border-white/10 bg-white/[0.035] p-1.5"><button type="button" aria-expanded={adminOpen} onClick={() => setAdminOpen((value) => !value)} className="flex min-h-[56px] w-full items-center gap-3 rounded-[15px] px-2.5 text-left"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-[14px] border border-white/[0.07] bg-white/[0.05] text-white/45"><Settings2 className="h-[18px] w-[18px]" /></span><span className="min-w-0 flex-1"><strong className="block text-[14px] font-semibold text-white">Администрирование и настройки</strong><span className="mt-0.5 block text-[12px] text-white/40">Интеграции, финансы, система и тестовые инструменты</span></span>{adminOpen ? <ChevronUp className="h-5 w-5 text-white/30" /> : <ChevronDown className="h-5 w-5 text-white/30" />}</button>{adminOpen ? <div data-testid="crm-more-service-tools" className="mt-1 border-t border-white/[0.07] pt-1">{adminItems.map((item) => <MenuRow key={item.id} {...item} />)}</div> : null}</section>
    <button type="button" onClick={() => void onLogout()} className="flex min-h-[52px] w-full items-center justify-center gap-2 rounded-[16px] border border-rose-300/15 bg-rose-300/[0.06] px-4 text-[14px] font-semibold text-rose-100/75"><LogOut className="h-[17px] w-[17px]" /> Выйти из кабинета организатора</button>
  </div>;
};
