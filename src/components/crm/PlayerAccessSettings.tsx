...
export function PlayerAccessSettings({ player, onSaved }: { player: PlayerDetails; onSaved?: () => void | Promise<void> }) {
  ...
  return (
    <>
      <section data-testid="crm-player-access-summary" className="rounded-[17px] border border-border-soft bg-surface-1 p-3.5">
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[12px] font-semibold text-text-primary">Доступ и роли</div>
        ...
        <div className="mt-3 grid grid-cols-3 gap-1.5 text-center">
          <div className="min-w-0 rounded-[11px] bg-black/20 px-1.5 py-2"><span className="block text-[8px] text-text-muted">Допуск</span><strong className="mt-1 block truncate text-[10px] font-semibold text-text-primary">{accessLabel(GAME_LEVELS, draft.game_level)}</strong></div>
          <div className="min-w-0 rounded-[11px] bg-black/20 px-1.5 py-2"><span className="block text-[8px] text-text-muted">Клуб</span><strong className="mt-1 block truncate text-[10px] font-semibold text-text-primary">{accessLabel(CLUB_ROLES, draft.club_role)}</strong></div>
          <div className="min-w-0 rounded-[11px] bg-black/20 px-1.5 py-2"><span className="block text-[8px] text-text-muted">Ведение</span><strong className="mt-1 block truncate text-[10px] font-semibold text-text-primary">{accessLabel(JUDGE_LEVELS, draft.judge_level)}</strong></div>
        </div>
      ...
      <MobileSheet open={open} onClose={() => setOpen(false)} title="Доступ и роли" subtitle={player.nickname} widthClass="sm:max-w-lg" footer={footer}>
