...
import { getRussianContactStatusLabel, getRussianEngagementStageLabel } from '../../lib/playerUtils.ts';
    ...
    <div className="min-w-0 space-y-3.5 sm:space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-[11px] text-text-muted sm:text-[13px] sm:text-text-secondary">
            {loading ? 'Загружаем список…' : `${players.length} человек · ${segmentCaption}`}
          </p>
        </div>
        <button type="button" onClick={() => { setAddError(null); setShowAddModal(true); }} className="inline-flex min-h-[44px] shrink-0 items-center gap-1.5 rounded-[12px] bg-white px-3 text-[12px] font-semibold text-[#090a0d]">
      ...
      <div className="flex gap-2">
        <label className="relative min-w-0 flex-1">
          ...
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Ник, имя, телефон или Telegram" className="mobile-field pl-10" />
        </label>
        <button type="button" onClick={() => setShowFilters(true)} className={`relative grid h-12 w-12 shrink-0 place-items-center rounded-[13px] border ${activeFilterCount ? 'border-accent bg-accent-soft text-accent' : 'border-border-soft bg-surface-1 text-text-secondary'}`} aria-label="Фильтры">
          <Filter className="h-5 w-5" />
          {activeFilterCount ? <span className="absolute right-1.5 top-1.5 grid h-4 min-w-4 place-items-center rounded-full bg-accent px-1 text-[9px] font-bold text-white">{activeFilterCount}</span> : null}
        </button>
      </div>
      ...
      <div className="-mx-1 overflow-x-auto px-1 pb-1">
        <div className="flex w-max min-w-full gap-2">
          {QUICK_FILTERS.map((item) => (
            <button key={item.id} type="button" onClick={() => setActiveQuickFilter(item.id)} className={`min-h-[44px] whitespace-nowrap rounded-full border px-4 text-[12px] font-semibold ${activeQuickFilter === item.id ? 'border-white/16 bg-white/[0.09] text-text-primary' : 'border-border-soft bg-surface-1 text-text-secondary'}`}>
              {item.label}
            </button>
          ))}
        </div>
      ...
      {listError ? (
        <div className="rounded-[14px] border border-danger/30 bg-danger-soft p-3 text-[12px] text-danger">
          <AlertCircle className="mr-1 inline h-4 w-4" /> {listError}
          <button type="button" onClick={() => void loadPlayers()} className="ml-2 font-bold underline">Повторить</button>
        </div>
      ) : null}
      ...
      {loading ? (
        ...
        <div className="rounded-[18px] border border-border-soft bg-surface-1 py-14 text-center">
          <UserRound className="mx-auto h-8 w-8 text-text-muted" />
          <p className="mt-3 text-[14px] font-semibold text-text-primary">В этом сегменте игроков нет</p>
        </div>
      ) : (
        <div data-testid="crm-active-player-list" className="overflow-hidden rounded-[18px] border border-border-soft bg-surface-1">
          {players.map((player, index) => {
            ...
            return (
              <button key={player.id} type="button" onClick={() => setLocalPlayerId(player.id)} className={`flex min-h-[76px] w-full items-center gap-3 px-3.5 py-2.5 text-left transition-colors active:bg-surface-hover ${index ? 'border-t border-border-soft' : ''}`}>
                <PlayerAvatar playerId={player.id} avatarVersion={player.avatar_updated_at} nickname={player.nickname} size="md" />
                <span className="min-w-0 flex-1">
                  <span className="flex min-w-0 items-center gap-2">
                    <strong className="min-w-0 truncate text-[14px] font-semibold leading-5 text-text-primary">{player.nickname}</strong>
                    <span className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-bold ${getPlayerActivitySegment(player) === 'loyal' ? 'bg-success-soft text-success' : getPlayerActivitySegment(player) === 'inactive' ? 'bg-warning-soft text-warning' : 'bg-white/[0.07] text-text-secondary'}`}>
                      {playerSegmentLabel(player)}
                    </span>
                  </span>
      ...
      <MobileSheet open={showFilters} onClose={() => setShowFilters(false)} title="Фильтры игроков" widthClass="sm:max-w-md">
        <div className="space-y-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
          <label className="block">
            ...
            <select value={contactStatusFilter} onChange={(event) => setContactStatusFilter(event.target.value)} className="mobile-field">
          ...
          <label className="block">
            <span className="mb-1.5 block text-[11px] font-semibold text-text-secondary">Активность</span>
            <select value={lifecycleStatus} onChange={(event) => setLifecycleStatus(event.target.value)} className="mobile-field">
              <option value="">По быстрому сегменту</option>
              ...
              <option value="lead">Ещё не играл</option>
            </select>
          </label>
          <div>
            <span className="mb-2 block text-[11px] font-semibold text-text-secondary">Дополнительно</span>
            <div className="grid grid-cols-2 gap-2">
              {([['', 'Без уточнения'], ['never', 'Не приходили'], ['absent60', '60+ дней'], ['open_tasks', 'Есть задачи']] as Array<[AdvancedSegment, string]>).map(([id, label]) => (
                <button key={id || 'all'} type="button" onClick={() => setAdvancedSegment(id)} className={`min-h-[44px] rounded-[11px] border px-3 text-[12px] font-semibold ${advancedSegment === id ? 'border-accent bg-accent-soft text-text-primary' : 'border-border-soft bg-surface-2 text-text-secondary'}`}>{label}</button>
              ))}
            </div>
          </div>
          <div className="rounded-[12px] border border-border-soft bg-surface-2 p-3 text-[11px] leading-5 text-text-muted">
            Активные — игроки с визитом за последние 45 дней. Лояльные — постоянные игроки с 4+ посещениями за этот же актуальный период.
          </div>
      ...
      <MobileSheet open={showAddModal} onClose={() => setShowAddModal(false)} title="Новый игрок" subtitle="Для начала достаточно никнейма. Остальное можно заполнить позже." widthClass="sm:max-w-md" footer={<button type="submit" form="focused-new-player-form" disabled={!newNickname.trim() || addSaving} className="min-h-[48px] w-full rounded-[13px] bg-accent text-[13px] font-bold text-white disabled:opacity-40">{addSaving ? 'Сохраняем…' : 'Добавить игрока'}</button>}>
        <form id="focused-new-player-form" onSubmit={handleCreatePlayer} className="space-y-3">
          {addError ? <div className="rounded-[13px] border border-danger/30 bg-danger-soft p-3 text-[12px] text-danger">{addError}</div> : null}
          <input value={newNickname} onChange={(event) => setNewNickname(event.target.value)} placeholder="Никнейм *" className="mobile-field" />
          <input value={newFullName} onChange={(event) => setNewFullName(event.target.value)} placeholder="Имя — необязательно" className="mobile-field" />
          <input value={newTgUsername} onChange={(event) => setNewTgUsername(event.target.value)} placeholder="Telegram" className="mobile-field" />
          <input value={newPhone} onChange={(event) => setNewPhone(event.target.value)} placeholder="Телефон" className="mobile-field" />
        </form>
      </MobileSheet>
      ...
      {selectedCardPlayerId ? (
        <div className="hidden" aria-hidden="true">
          <PlayersCRM
            ...
            onClosePlayerCard={handleCardClose}
            onCrmChanged={() => { void loadPlayers(); onCrmChanged?.(); }}
          />
        </div>
      ) : null}
    </div>
