...
export default function TableScoutingCard({ participantIds }: { participantIds: string[] }) {
  ...
  return <div className="rounded-2xl border border-accent/15 bg-accent-soft/30 p-3 text-text-primary">
    <button type="button" onClick={() => setExpanded((value) => !value)} className="flex w-full items-start justify-between gap-3 text-left">
      <span><span className="block text-[10px] font-black uppercase tracking-[0.13em] text-text-muted">🔎 Разведка стола</span><span className="mt-1 block text-[12px] font-black">{data.summary.strength_label}</span><span className="mt-0.5 block text-[10px] text-text-muted">{data.summary.experience_label}</span></span>
      <span className="text-[10px] text-text-muted">{expanded ? '⌃' : '⌄'}</span>
    </button>

    <div className="mt-3 grid grid-cols-4 gap-1.5 text-center"><div className="rounded-xl bg-surface-1 p-2"><div className="text-[13px] font-black">{data.summary.average_elo}</div><div className="text-[10px] text-text-muted">ср. Elo</div></div><div className="rounded-xl bg-surface-1 p-2"><div className="text-[13px] font-black">{data.summary.average_games}</div><div className="text-[10px] text-text-muted">ср. игр</div></div><div className="rounded-xl bg-surface-1 p-2"><div className="text-[13px] font-black">{data.summary.newcomers}</div><div className="text-[10px] text-text-muted">до 5 игр</div></div><div className="rounded-xl bg-surface-1 p-2"><div className="text-[13px] font-black">{data.summary.hot_players}</div><div className="text-[10px] text-text-muted">в форме</div></div></div>

    {expanded && <>
      <div className="mt-3 space-y-1.5">{data.players.map((player) => <div key={player.participant_id} className="flex items-center gap-2 rounded-xl bg-surface-1 px-2.5 py-2"><img src={player.avatar_url} alt="" onError={(event) => { event.currentTarget.style.display = 'none'; }} className="h-8 w-8 shrink-0 rounded-lg object-cover" /><div className="min-w-0 flex-1"><div className="truncate text-[10px] font-bold">{player.nickname}</div><div className="mt-0.5 flex gap-1">{player.recent_form.map((won, index) => <span key={index} className={`h-1.5 w-1.5 rounded-full ${won ? 'bg-success' : 'bg-danger'}`} />)}<span className="ml-1 text-[10px] text-text-muted">{countGames(player.games)} · {player.win_rate}%</span></div></div><div className="text-[10px] font-black">{Math.round(player.elo)}</div></div>)}</div>
      <div className="mt-3 space-y-1">{data.notes.map((note, index) => <div key={index} className="text-[10px] leading-4 text-text-secondary">• {note}</div>)}</div>
      <p className="mt-2 text-[10px] leading-3 text-text-muted">{data.meta?.safety}</p>
    </>}
  </div>;
}
