...
export function TournamentEveningSettingsPanel({ tournamentId, onChanged }: { tournamentId: string; onChanged?: () => void }) {
  ...
  return <section className="rounded-[18px] border border-border-soft bg-surface-1 p-3.5 sm:p-4">
    ...
    <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
      ...
      {detail.published_at && (detail.lifecycle === 'registration_open' || detail.lifecycle === 'registration_closed') ? <button type="button" disabled={busy} onClick={() => void requestAction('/publish-and-notify')} className="min-h-11 rounded-xl border border-border-soft bg-surface-2 text-xs font-bold" data-testid="retry-tournament-notifications">Повторить уведомления игрокам</button> : null}
