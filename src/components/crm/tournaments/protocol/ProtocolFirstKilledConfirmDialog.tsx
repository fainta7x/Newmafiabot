...
type ProtocolFirstKilledConfirmDialogProps = {
  isOpen: boolean;
  onCancel: () => void;
  onConfirm: () => void;
};

export function ProtocolFirstKilledConfirmDialog({
  isOpen,
  onCancel,
  onConfirm,
}: ProtocolFirstKilledConfirmDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 max-w-md w-full space-y-4 text-slate-100 shadow-2xl">
        ...
        <p className="text-xs sm:text-sm text-slate-300">Выбранный ЛХ и ручной Ci прежнего первоубиенного будут очищены.</p>
