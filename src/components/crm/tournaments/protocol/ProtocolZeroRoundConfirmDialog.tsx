  ...
  return (
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 max-w-md w-full space-y-4 text-slate-100 shadow-2xl">
        ...
        <p className="text-xs sm:text-sm text-slate-300">Выбранные номера ЛХ прежнего игрока будут очищены.</p>
        <div className="flex items-center justify-end space-x-2 pt-2">
          ...
          <button type="button" onClick={onConfirm} className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs shadow-md">Сменить и очистить</button>
        </div>
      </div>
    </div>
  );
