  ...
  return <div className="mt-5 space-y-3">
    <section className="rounded-[24px] border border-violet-200/10 bg-gradient-to-br from-violet-300/[0.07] to-white/[0.02] p-4">
      <div className="flex items-start justify-between gap-3">
        <div><div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-violet-100/50">🗞 Неделя клуба</div><div className="mt-1 text-lg font-black">Что произошло за 7 дней</div><div className="mt-1 text-[10px] text-white/30">{countGames(data.weekly_digest.games)} · {countPlayers(data.weekly_digest.players)}</div></div>
    ...
    <div className="rounded-[24px] border border-sky-200/10 bg-gradient-to-br from-sky-300/[0.055] to-white/[0.02] p-4">
      <div className="flex items-start justify-between gap-3"><div><div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-sky-100/45">Сезон клуба</div><div className="mt-1 text-lg font-black">{data.season.label}</div><div className="mt-1 text-[10px] text-white/30">{countGames(data.season.games)} · {countPlayers(data.season.players)}</div></div>{viewerRank && <div className="rounded-2xl bg-black/20 px-3 py-2 text-center"><div className="text-xl font-black">#{viewerRank.place}</div><div className="text-[8px] text-white/25">ваше место</div></div>}</div>
