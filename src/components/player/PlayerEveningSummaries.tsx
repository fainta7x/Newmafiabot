...
export default function PlayerEveningSummaries({
  ...
  if (!summaries && !error) return <main className={`grid ${embedded ? 'min-h-[46vh]' : 'min-h-screen'} place-items-center bg-[#090a0d] text-white`}><div className="text-center"><div className="text-3xl">🎬</div><div className="mt-2 text-xs text-white/30">Собираем итоги вечеров…</div></div></main>;
    ...
    <main className={`min-h-screen bg-[#090a0d] px-3 pb-28 ${embedded ? 'pt-2' : 'pt-3'} text-white`}>
      <div className="mx-auto w-full max-w-[430px] space-y-3">
        ...
        {!summaries?.length ? <div className="rounded-3xl border border-white/[0.06] bg-white/[0.03] p-8 text-center"><div className="text-3xl">🎭</div><div className="mt-2 text-sm font-semibold">Итогов пока нет</div><p className="mt-1 text-xs text-white/30">После завершённого вечера, на котором ты был или играл, он появится здесь.</p></div> : <>
          {summaries.length > 1 && <div className="flex gap-2 overflow-x-auto pb-1">{summaries.map((summary) => <button key={summary.id} type="button" onClick={() => setSelectedId(summary.id)} className={`shrink-0 rounded-2xl border px-3 py-2 text-left ${selectedId === summary.id ? 'border-white/20 bg-white text-black' : 'border-white/[0.06] bg-white/[0.035] text-white/55'}`}><div className="text-[11px] font-semibold">{dateText(summary.starts_at)}</div><div className="mt-0.5 text-[11px] opacity-60">{countGames(summary.games)} · {summary.red_wins}:{summary.black_wins}</div></button>)}</div>}
          ...
          {selected && <>
            <section className="overflow-hidden rounded-[30px] border border-white/10 bg-gradient-to-br from-violet-300/[0.08] via-white/[0.035] to-rose-300/[0.04] p-5 text-center">
              <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/30">{dateText(selected.starts_at)}{selected.venue ? ` · ${selected.venue}` : ''}</div>
              <h2 className="mt-2 text-xl font-black">{selected.title}</h2>
              <div className="mt-5 flex items-end justify-center gap-5"><div><div className="text-[11px] text-rose-200/45">КРАСНЫЕ</div><div className="mt-1 text-5xl font-black text-rose-200">{selected.red_wins}</div></div><div className="pb-2 text-xl font-black text-white/20">:</div><div><div className="text-[11px] text-white/35">ЧЁРНЫЕ</div><div className="mt-1 text-5xl font-black">{selected.black_wins}</div></div></div>
              <div className="mt-3 text-[11px] text-white/30">{selected.games} завершённых игр</div>
            </section>

            <section className="grid grid-cols-3 gap-2 text-center"><div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-3"><div className="text-xl font-black">{selected.player.games}</div><div className="mt-1 text-[11px] text-white/25">твоих игр</div></div><div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-3"><div className="text-xl font-black">{selected.player.win_rate}%</div><div className="mt-1 text-[11px] text-white/25">побед</div></div><div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-3"><div className={`text-xl font-black ${selected.player.elo_delta > 0 ? 'text-emerald-300' : selected.player.elo_delta < 0 ? 'text-rose-300' : 'text-white/55'}`}>{signed(selected.player.elo_delta)}</div><div className="mt-1 text-[11px] text-white/25">Elo</div></div></section>

            {selected.player.elo_before != null && selected.player.elo_after != null && <section className="rounded-2xl border border-amber-200/10 bg-amber-200/[0.03] px-4 py-3"><div className="flex items-center justify-between"><div><div className="text-[11px] uppercase tracking-[0.14em] text-amber-100/35">Твой рейтинг за вечер</div><div className="mt-1 text-sm font-semibold">{Math.round(selected.player.elo_before)} → {Math.round(selected.player.elo_after)}</div></div><div className={`text-xl font-black ${selected.player.elo_delta > 0 ? 'text-emerald-300' : selected.player.elo_delta < 0 ? 'text-rose-300' : 'text-white/45'}`}>{signed(selected.player.elo_delta)}</div></div></section>}

            {selected.facts.length > 0 && <section className="rounded-[24px] border border-white/[0.06] bg-white/[0.025] p-4"><div className="text-[11px] font-semibold uppercase tracking-[0.15em] text-white/35">Что запомнить</div><div className="mt-3 space-y-2">{selected.facts.map((fact) => <div key={fact} className="flex gap-2 text-xs leading-5 text-white/55"><span className="text-amber-200/55">✦</span><span>{fact}</span></div>)}</div></section>}

            {selected.awards.length > 0 && <section className="rounded-[24px] border border-amber-200/10 bg-amber-200/[0.025] p-4"><div className="text-[11px] font-semibold uppercase tracking-[0.15em] text-amber-100/40">Выбор игроков</div><div className="mt-3 grid grid-cols-2 gap-2">{selected.awards.map((award) => <div key={award.category} className="rounded-2xl bg-black/20 p-3"><div className="flex items-center gap-2"><Avatar src={award.avatar_url} /><div className="min-w-0"><div className="truncate text-[11px] font-semibold">{award.nickname}</div><div className="mt-0.5 text-[11px] text-white/25">{award.label}</div></div></div><div className="mt-2 text-[11px] text-white/25">{award.votes} голосов</div></div>)}</div></section>}

            <section className="grid grid-cols-2 gap-2">{selected.best_elo_rise && <div className="rounded-2xl border border-emerald-200/10 bg-emerald-200/[0.03] p-3"><div className="text-[11px] text-emerald-100/40">📈 Рост вечера</div><div className="mt-2 flex items-center gap-2"><Avatar src={selected.best_elo_rise.avatar_url} /><div className="min-w-0"><div className="truncate text-[11px] font-semibold">{selected.best_elo_rise.nickname}</div><div className="text-sm font-black text-emerald-300">{signed(selected.best_elo_rise.elo_delta)}</div></div></div></div>}{selected.most_games && <div className="rounded-2xl border border-white/[0.06] bg-white/[0.025] p-3"><div className="text-[11px] text-white/30">🎟 Больше всех игр</div><div className="mt-2 flex items-center gap-2"><Avatar src={selected.most_games.avatar_url} /><div className="min-w-0"><div className="truncate text-[11px] font-semibold">{selected.most_games.nickname}</div><div className="text-sm font-black">{selected.most_games.games}</div></div></div></div>}</section>
