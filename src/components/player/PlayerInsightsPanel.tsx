...
export default function PlayerInsightsPanel() {
  ...
  return <div className="mt-5 rounded-[22px] border border-white/[0.06] bg-white/[0.022] p-3">
    ...
    {expanded && <>
      ...
      <p className="mt-1 text-[11px] leading-3 text-white/18">{data.meta?.opponents}</p>

      <div className="mt-4"><div className="text-[11px] font-semibold uppercase tracking-[0.13em] text-white/30">🕸 Социальный граф клуба</div><div className="mt-0.5 text-[11px] text-white/20">Ближе — больше совместных игр</div><div className="mt-2"><SocialGraph {...data.social_graph} /></div></div>
    </>}
  </div>;
}
