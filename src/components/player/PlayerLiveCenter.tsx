...
function JourneySteps({ phase }: { phase: 'upcoming' | 'live' | 'recap' }) {
  ...
  return <div className="grid grid-cols-4 gap-1.5">{steps.map((label, index) => (
    <div key={label} className="min-w-0 text-center">
      <div className={`mx-auto grid h-6 w-6 place-items-center rounded-full text-[11px] font-black ${index < current ? 'bg-emerald-400/80 text-black' : index === current ? 'bg-white text-black' : 'bg-white/[0.06] text-white/25'}`}>{index < current ? '✓' : index + 1}</div>
      <div className={`mt-1 truncate text-[11px] font-semibold ${index === current ? 'text-white/70' : 'text-white/25'}`}>{label}</div>
    </div>
  ))}</div>;
}
...
export default function PlayerLiveCenter({ compact = false }: { compact?: boolean }) {
  ...
  return <>
    <button
      data-testid="player-live-launcher"
      type="button"
    ...
    >
      <span className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-2 py-1 text-[11px] font-black uppercase tracking-[0.14em] ${isLive ? 'bg-rose-500/15 text-rose-300' : isRecap ? 'bg-amber-200/10 text-amber-100/70' : 'bg-white/[0.07] text-white/55'}`}>
      ...
      <span className="min-w-0 flex-1">
        ...
        {!compact ? <span className="mt-0.5 block truncate text-[11px] text-white/38">{compactMeta}</span> : null}
      </span>
      ...
      <section role="dialog" aria-modal="true" aria-label="Игровой вечер" onClick={(event) => event.stopPropagation()} className="max-h-[88dvh] w-full max-w-[430px] overflow-y-auto rounded-t-[28px] border border-white/10 bg-[#111217] p-4 pb-[max(1rem,env(safe-area-inset-bottom))] text-white shadow-2xl sm:rounded-[28px]">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/28">2LA Noire · один вечер</div>
        ...
        {journey.phase === 'upcoming' && <>
          <section className="mt-3 rounded-[24px] border border-white/10 bg-gradient-to-b from-white/[0.07] to-white/[0.025] p-4">
            <div className="flex items-start justify-between gap-3"><div><div className="text-[11px] uppercase tracking-[0.14em] text-white/30">До вечера</div><div className="mt-1 text-lg font-semibold">{responseLabel(journey.participation.response_status)}</div></div><div className="text-right"><div className="text-lg font-black">{journey.attending_count}</div><div className="text-[11px] text-white/25">уже идут</div></div></div>
          ...
          <div className="mt-3 rounded-2xl bg-white/[0.03] px-3 py-3 text-[11px] leading-4 text-white/32">Когда организатор начнёт вечер, этот же экран автоматически переключится на явку, текущую десятку и счёт. Ничего искать заново не придётся.</div>
        </>}

        {journey.phase === 'live' && <>
          <section className="mt-3 rounded-[24px] border border-white/10 bg-gradient-to-b from-white/[0.08] to-white/[0.035] p-4">
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 text-center"><div><div className="text-[11px] uppercase tracking-[0.14em] text-rose-300/70">Красные</div><div className="mt-1 text-4xl font-black text-rose-300">{journey.score.red}</div></div><div className="text-xl text-white/20">:</div><div><div className="text-[11px] uppercase tracking-[0.14em] text-white/40">Чёрные</div><div className="mt-1 text-4xl font-black">{journey.score.black}</div></div></div>
            <div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-2xl bg-black/20 p-3 text-center"><div className="text-lg font-semibold">{journey.score.completed}</div><div className="text-[11px] text-white/30">игр завершено</div></div><div className="rounded-2xl bg-black/20 p-3 text-center"><div className="text-lg font-semibold">{journey.present_count}</div><div className="text-[11px] text-white/30">в клубе</div></div></div>
          </section>
          ...
          <section className={`mt-3 rounded-[22px] border p-4 ${journey.participation.state === 'playing' ? 'border-amber-200/15 bg-amber-200/[0.04]' : 'border-white/[0.06] bg-white/[0.025]'}`}>
            <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-white/30">Твой статус сейчас</div>
          ...
          <section className="mt-3 rounded-[22px] border border-white/[0.06] bg-white/[0.025] p-4">
            {journey.current_game ? <><div className="flex items-start justify-between gap-3"><div><div className="text-[11px] uppercase tracking-[0.14em] text-rose-300/60">Сейчас за столом</div><div className="mt-1 text-lg font-semibold">Игра {journey.current_game.local_number}</div><div className="mt-1 text-[11px] text-white/30">{[journey.current_game.table_name, journey.current_game.judge_name ? `ведущий ${journey.current_game.judge_name}` : null].filter(Boolean).join(' · ') || 'Состав сформирован'}</div></div><span className="rounded-full bg-rose-500/10 px-2 py-1 text-[11px] font-semibold text-rose-300">в процессе</span></div><div className="mt-3 grid grid-cols-2 gap-1.5">{journey.current_game.players.map((player) => <div key={`${journey.current_game!.id}:${player.seat_number}`} className={`flex min-w-0 items-center gap-2 rounded-xl px-2.5 py-2 ${player.player_id && journey.participation.state === 'playing' && player.seat_number === journey.participation.seat_number ? 'bg-amber-200/[0.08]' : 'bg-black/20'}`}><span className="w-5 shrink-0 text-center text-[11px] font-mono text-white/28">{player.seat_number}</span><span className="truncate text-[11px] font-medium text-white/75">{player.nickname}</span></div>)}</div><p className="mt-3 text-[11px] leading-4 text-white/22">Роли и закрытая информация до завершения партии здесь не показываются.</p></> : <div className="py-4 text-center"><div className="text-2xl">☕</div><div className="mt-2 text-sm font-semibold">Между играми</div><div className="mt-1 text-xs text-white/30">Следующая десятка появится здесь автоматически.</div></div>}
          </section>

          {journey.recent_results.length > 0 && <section className="mt-3"><div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/30">После игры</div><div className="space-y-1.5">{journey.recent_results.map((result) => <button key={result.id} type="button" onClick={() => navigate(`/player/replay/${encodeURIComponent(result.game_key)}`)} className={`flex w-full items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-left ${result.self_played ? 'border border-white/10 bg-white/[0.06]' : 'bg-white/[0.035]'}`}><div><div className="text-xs font-medium">Игра {result.local_number}{result.self_played ? ' · ты играл' : ''}</div><div className="mt-0.5 text-[11px] text-white/25">{result.self_played && result.self_won != null ? (result.self_won ? 'Твоя победа' : 'Твоё поражение') : 'Открыть Replay'}</div></div><div className="text-xs font-semibold text-white/60">{winnerText(result.winner_team)} ›</div></button>)}</div></section>}
        </>}

        {journey.phase === 'recap' && <>
          <section className="mt-3 rounded-[26px] border border-amber-200/10 bg-gradient-to-b from-amber-200/[0.05] to-white/[0.025] p-5 text-center"><div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-amber-100/45">Вечер завершён</div><div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-4"><div><div className="text-[11px] text-rose-300/60">КРАСНЫЕ</div><div className="text-4xl font-black text-rose-300">{journey.recap.red_wins}</div></div><div className="text-xl text-white/15">:</div><div><div className="text-[11px] text-white/35">ЧЁРНЫЕ</div><div className="text-4xl font-black">{journey.recap.black_wins}</div></div></div><div className="mt-4 grid grid-cols-3 gap-1.5"><div className="rounded-xl bg-black/20 p-2"><div className="text-lg font-black">{journey.recap.player.games}</div><div className="text-[11px] text-white/25">твоих игр</div></div><div className="rounded-xl bg-black/20 p-2"><div className="text-lg font-black">{journey.recap.player.wins}</div><div className="text-[11px] text-white/25">побед</div></div><div className="rounded-xl bg-black/20 p-2"><div className={`text-lg font-black ${journey.recap.player.elo_delta > 0 ? 'text-emerald-300' : journey.recap.player.elo_delta < 0 ? 'text-rose-300' : ''}`}>{journey.recap.player.elo_delta > 0 ? '+' : ''}{journey.recap.player.elo_delta}</div><div className="text-[11px] text-white/25">Elo</div></div></div></section>
          {journey.recap.facts.length > 0 && <div className="mt-3 space-y-1.5">{journey.recap.facts.slice(0, 4).map((fact, index) => <div key={index} className="rounded-xl bg-white/[0.035] px-3 py-2 text-[11px] leading-4 text-white/40">{fact}</div>)}</div>}
