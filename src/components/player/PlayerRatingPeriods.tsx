...
function ScoreBadge({ label, value }: { label: string; value: number }) {
  ...
  return (
    <span className={`rounded-full px-2 py-1 text-[11px] ${value > 0 ? 'bg-emerald-400/10 text-emerald-200/80' : 'bg-rose-400/10 text-rose-200/80'}`}>
          ...
          {periods.active_periods.map((period) => period.id === selectedPeriodId ? (
            <div key={period.id}>
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="shrink-0 rounded-full bg-emerald-400/10 px-2 py-1 text-[11px] font-medium text-emerald-200/80">идёт сейчас</span>
              </div>
          ...
          {!periods.active_periods.some((period) => period.id === detail.period.id) && (
            <Section title="Архивный период">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="shrink-0 rounded-full bg-white/[0.07] px-2 py-1 text-[11px] text-white/50">завершён</span>
              </div>
            </Section>
          )}
          ...
          {selectedPlayer ? (
            <Section title="Результат игрока">
              ...
              <div className="flex items-center gap-3 rounded-2xl bg-black/20 p-3">
                ...
                <div className="shrink-0 text-right"><div className="text-xl font-semibold text-white">{scoreNumber(selectedPlayer.total_points)}</div><div className="text-[11px] uppercase tracking-[0.12em] text-white/30">баллов</div></div>
              </div>
              <div className="mt-2 grid grid-cols-3 gap-2"><div className="rounded-2xl bg-black/20 p-3"><div className="text-lg font-semibold">#{selectedPlayer.place}</div><div className="mt-1 text-[11px] text-white/35">место</div></div><div className="rounded-2xl bg-black/20 p-3"><div className="text-lg font-semibold">{signedScore(selectedPlayer.additional_total)}</div><div className="mt-1 text-[11px] text-white/35">доп. баллы</div></div><div className="rounded-2xl bg-black/20 p-3"><div className="text-lg font-semibold">{scoreNumber(selectedPlayer.best_move_points)}</div><div className="mt-1 text-[11px] text-white/35">ЛХ</div></div></div>
                ...
                {selfStanding ? (
                  <>
                    <div className="grid grid-cols-4 gap-1.5 text-center">
                      <div className="rounded-xl bg-black/20 p-2"><div className="text-xl font-semibold">#{selfStanding.place}</div><div className="mt-1 text-[11px] text-white/35">место</div></div>
                      <div className="rounded-xl bg-black/20 p-2"><div className="text-xl font-semibold">{scoreNumber(selfStanding.total_points)}</div><div className="mt-1 text-[11px] text-white/35">баллов</div></div>
                      <div className="rounded-xl bg-black/20 p-2"><div className="text-xl font-semibold">{selfStanding.games_played}</div><div className="mt-1 text-[11px] text-white/35">игр</div></div>
                      <div className="rounded-xl bg-black/20 p-2"><div className="text-xl font-semibold">{selfStanding.wins}</div><div className="mt-1 text-[11px] text-white/35">побед</div></div>
                    </div>
                  ...
                  <div className="space-y-2">{detail.standings.map((item) => {
                    ...
                    return (
                      <button key={item.player_id} type="button" onClick={() => setSelectedPlayerId(item.player_id)} className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-left ${isSelf ? 'border-white/20 bg-white/[0.08]' : 'border-transparent bg-black/20'}`}>
                        ...
                        <div className="min-w-0 flex-1"><div className="truncate text-sm font-medium text-white">{item.nickname}{isSelf ? ' · вы' : ''}</div><div className="mt-1 text-[11px] text-white/35">{countGames(item.games_played)} · {countWins(item.wins)} · доп. {signedScore(item.additional_total)}</div></div>
                        <div className="shrink-0 text-right"><div className="text-base font-semibold text-white">{scoreNumber(item.total_points)}</div><div className="text-[11px] text-white/30">баллов ›</div></div>
                      </button>
                    );
                  })}</div>
