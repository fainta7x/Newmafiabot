...
type TournamentDetail = {
  ...
  prize_allocations: Array<{ place: string; amount_rub: number }>;
  notes?: string | null;
  me?: TournamentRegistration | null;
};
...
const paymentLabel = (state?: TournamentRegistration['payment_state']) => ({
  unpaid: 'Не подтверждён',
  pending: 'Ждёт проверки организатора',
  confirmed: 'Оплата подтверждена',
  rejected: 'Нужно уточнить оплату',
  waived: 'Взнос не требуется',
  refunded: 'Возвращено',
}[state || 'unpaid']);
...
const registrationLabel = (registration?: TournamentRegistration | null) => {
  ...
  if (registration.status === 'reserve') return `Резерв · №${Number(registration.queue_order || 0)}`;
  if (registration.status === 'declined') return 'Регистрация отклонена';
  return 'Регистрация отменена';
};

export default function PlayerTournamentEveningDetail({ tournamentId, onBack, onSaved }: {
  tournamentId: string;
  onBack: () => void;
  onSaved: () => void;
}) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDetail(body as TournamentDetail);
    } catch (loadError: any) {
      setError(loadError?.message || 'Не удалось загрузить турнир');
    } finally {
      setLoading(false);
    }
  };
        ...
        {detail ? <>
          ...
          <section className="mt-3 grid grid-cols-2 gap-2">
            <div className="rounded-2xl border border-white/[0.07] bg-white/[0.035] p-3">
              <div className="text-[10px] uppercase tracking-[0.12em] text-white/30">Состав</div>
              <div className="mt-1 text-lg font-black">{detail.confirmed_count}/{detail.player_capacity}</div>
              <div className="mt-1 text-xs text-white/40">Свободно: {detail.remaining_places}</div>
            </div>
            <div className="rounded-2xl border border-white/[0.07] bg-white/[0.035] p-3">
          ...
          <section className="mt-3 rounded-[24px] border border-white/[0.07] bg-white/[0.035] p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-[10px] uppercase tracking-[0.12em] text-white/30">Призовой фонд</div>
                <div className="mt-1 text-xl font-black">{Number(detail.prize_fund_rub || 0).toLocaleString('ru-RU')} ₽</div>
              </div>
            </div>
            {detail.prize_allocations?.length ? <div className="mt-3 space-y-1.5">
              {detail.prize_allocations.map((item, index) => <div key={`${item.place}-${index}`} className="flex items-center justify-between rounded-xl bg-black/20 px-3 py-2 text-sm"><span className="text-white/55">{item.place}</span><b>{Number(item.amount_rub || 0).toLocaleString('ru-RU')} ₽</b></div>)}
            </div> : <div className="mt-2 text-xs text-white/35">Распределение призов не указано.</div>}
          </section>
