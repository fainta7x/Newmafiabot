...
const MIGRATION_KEY = '0016_fix_sep4_chagin_game_identity_v1';
...
const resultLooksLikeChagin = (result: any, playerNicknameById: Map<string, string>) => {
  if (CHAGIN_ALIASES.has(normalizeNickname(result?.display_name))) return true;
  const playerNickname = playerNicknameById.get(String(result?.player_id || ''));
  return CHAGIN_ALIASES.has(normalizeNickname(playerNickname));
};
...
export interface Sep4ChaginRepairResult {
  applied: boolean;
  gamesChanged: number[];
  eveningId: string | null;
}

export async function applySep4ChaginGameIdentityMigration(db: DatabaseWrapper): Promise<Sep4ChaginRepairResult> {
  ...
  for (const evening of candidateEvenings) {
    ...
    const games = rows.map((row) => {
      ...
      const chaginResults = results.filter((result: any) => resultLooksLikeChagin(result, playerNicknameById));
      return { row, protocol, slots, results, chaginResults };
    });
    if (games.some((game) => game.chaginResults.length)) inspected.push({ evening, games });
  }

  if (inspected.length !== 1) {
    throw new Error(`[DATA] Expected exactly one relevant ${TARGET_DATE} evening containing Chagin games, found ${inspected.length}`);
  }
  ...
  if (secondGame.chaginResults.length !== 1) {
    throw new Error(`[DATA] Expected exactly one Chagin seat in the second ${TARGET_DATE} game, found ${secondGame.chaginResults.length}`);
  }
  for (const game of target.games.slice(2)) {
    ...
    if (game.chaginResults.length === 1 && game.results.some((result: any) => String(result?.player_id || '') === String(fandorin.id))) {
  ...
  const applyToGame = async (game: any, replacement: { participantId: string; playerId: string; nickname: string }) => {
    const oldResult = game.chaginResults[0];
    if (!oldResult) return;
    const oldParticipantId = String(oldResult.participant_id || '');
    const oldPlayerId = String(oldResult.player_id || '');
    if (!oldParticipantId || !oldPlayerId) throw new Error(`[DATA] Game ${game.row.id} Chagin result is missing identity`);
    if (oldPlayerId !== String(chagin.id)) {
      throw new Error(`[DATA] Game ${game.row.id} label looks like Chagin but player_id does not match canonical Chagin; automatic replacement aborted`);
    }
  ...
  console.log(`[DATA] Corrected ${gamesChanged.length} Chagin seat identity record(s) from ${TARGET_DATE}.`);
