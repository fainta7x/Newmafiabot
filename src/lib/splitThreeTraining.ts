 ...
 * less, and that one has a black check. Nobody knows which sheriff is real, so there are two versions:
 * if a sheriff is real, the other sheriff and everyone he checked black are mafia.
 * - Who is split: the less trusted sheriff, his black check and one more player; if the trusted
 *   sheriff also has a black check, the split is both black checks and the less trusted sheriff.
 * - Who votes where: first the hands that could break the split. Everyone who is black by one version
 *   votes for a split player who is black by the other version (first such nominee in nomination order
 *   with a free vote). As mafia they then vote against a red player and have no spare hand to break
 *   the split. Then the other split players vote for the first split player with a free vote (even
 *   themselves), then everyone else in seat order fills the nominees in nomination order.
...
export type SplitThreeSheriffs = {
  ...
  /** The sheriff the town trusts less; his check is always black. */
  doubted: { seat: number; check: number };
};
...
export const SPLIT_THREE_HARD_RULES = [
  ...
  'Пилятся шериф, которому город верит меньше, его чёрная проверка и ещё один игрок. Если чёрная проверка есть и у второго шерифа — пилятся обе чёрные проверки и шериф, которому верят меньше.',
  'Сначала ставим руки, которые могут сломать попил: кто чёрный по одной версии, голосует в пилящегося, который чёрный по другой версии. Если они мафия, они голосуют против красного, и лишней руки у них нет.',
  'Потом остальные пилящиеся голосуют в первого пилящегося, в котором есть место (даже в себя), а остальные добивают кандидатов по порядку мест.',
] as const;
...
export const blackIfReal = (sheriffs: SplitThreeSheriffs, sheriff: 'trusted' | 'doubted'): number[] => (sheriff === 'trusted'
  ...
  : [sheriffs.trusted.seat, sheriffs.doubted.check]).sort((a, b) => a - b);
...
const hardAssignments = (scenario: SplitThreeScenario): Record<number, number[]> => {
  ...
  // 1. The hands that could break the split.
  const constrained = splitThreeVersions(scenario).flatMap(({ blacks, into }) => blacks.map((voter) => ({ voter, into })))
    .sort((a, b) => a.voter - b.voter);
  for (const { voter, into } of constrained) if (!used.has(voter)) place(voter, into);
  // 2. The other split players, then 3. everyone else in seat order.
  for (const voter of [...scenario.split].sort((a, b) => a - b)) if (!used.has(voter)) place(voter, scenario.split);
...
const hardSplitSeats = (sheriffs: SplitThreeSheriffs) => (sheriffs.trusted.black
  ? [sheriffs.trusted.check, sheriffs.doubted.check, sheriffs.doubted.seat]
  : [sheriffs.doubted.seat, sheriffs.doubted.check]);
...
const generateHardScenario = (random: () => number): SplitThreeScenario => {
  ...
  const sheriffs: SplitThreeSheriffs = { trusted: { seat: trusted, check: trustedCheck, black: random() < 0.5 }, doubted: { seat: doubted, check: doubtedCheck } };
...
const isValidHardClaims = (value: unknown, killed: number, split: number[]) => {
  ...
  if (!trusted || !doubted || typeof trusted.black !== 'boolean') return false;
  ...
  return required.length === 3 || !split.some((seat) => seat === trusted.seat || seat === trusted.check);
};
