...
export const SPLIT_VOTE_RULES = [
  ...
  'Двое из одной половины (№2–5 или №6–10): больший номер и четыре следующих за ним голосуют в меньший номер; остальные — в больший.',
] as const;
...
export const splitVoteGroups = ([first, second]: [number, number]): { first: number[]; second: number[] } => {
  ...
  const firstVoters = first === 1
    ...
    : first <= 5 && second >= 6
      ? [6, 7, 8, 9, 10]
      : SEATS.filter((seat) => (seat - second + 10) % 10 < 5);
...
export const generateSplitVoteScenario = (previous?: SplitVoteScenario, random: () => number = Math.random): SplitVoteScenario => {
  const count = pick([2, 3, 4, 5].filter((value) => value !== previous?.candidates.length), random);
