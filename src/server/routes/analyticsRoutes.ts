...
router.get('/staff', requireOrganizerAuth, async (req, res) => {
  try {
    ...
    const days = ({ '7d': 7, '30d': 30, '90d': 90 } as Record<string, number>)[String(req.query.period || 'all')] || 0;
    const since = days ? new Date(Date.now() - days * 86_400_000).toISOString() : '0000';
    ...
    if (tables.has('evening_staff_assignments')) {
      for (const row of await db.all<any>(`
        SELECT s.organizer_player_id AS id, p.nickname, COUNT(*) AS count
         ...
         WHERE (e.status = 'completed' OR e.settled_at IS NOT NULL) AND e.starts_at >= ?
         GROUP BY s.organizer_player_id, p.nickname`, [since])) bump(String(row.id), String(row.nickname), 'evenings', Number(row.count));
    }
    const clubGames = await db.all<any>(`
      SELECT g.judge_player_id AS id, p.nickname, g.protocol_text
       ...
       WHERE g.archived_at IS NULL AND COALESCE(e.starts_at, g.created_at) >= ?`, [since]);
    ...
    if (tables.has('tournament_games')) {
      for (const row of await db.all<any>(`
        SELECT tg.judge_player_id AS id, p.nickname, COUNT(*) AS count
          FROM tournament_games tg JOIN players p ON p.id = tg.judge_player_id
         WHERE tg.status = 'completed' AND COALESCE(tg.completed_at, '') >= ?
         GROUP BY tg.judge_player_id, p.nickname`, [since])) bump(String(row.id), String(row.nickname), 'games', Number(row.count));
    }
    const staff = [...rows.values()].sort((a, b) => (b.evenings + b.games) - (a.evenings + a.games) || a.nickname.localeCompare(b.nickname, 'ru'));
    return res.json({ period: req.query.period || 'all', staff });
  } catch (error: any) {
