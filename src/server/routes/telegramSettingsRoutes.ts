...
import {
  enqueueOrganizerTestNotification,
  getOrganizerNotificationDiagnostics,
} from '../services/organizerNotificationService.ts';
...
router.get('/', async (req, res) => {
  try {
    ...
    const organizerNotifications = await getOrganizerNotificationDiagnostics(db);
    res.json({
      ...
      runtime_monitor: {
        separately_configured: true,
        note: 'Webhook health checks Telegram runtime only. GitHub monitor secrets must be verified in GitHub Actions separately.',
      },
    });
  } catch (error: any) {
...
router.get('/evenings/:eveningId', async (req, res) => {
  try {
    ...
    const desiredIds = ['published', 'active'].includes(String(evening.status)) && !evening.settled_at
      ...
      : [];
    const destinations = await db.all(`SELECT id, name, chat_id, topic_id, active FROM telegram_destinations`);
    ...
    res.json({
      ...
      destinations: desiredIds.map((id) => {
        ...
        const publication: any = pubById.get(id) || null;
        return {
          id,
          name: config.name || id,
...
router.patch('/:destinationId', async (req, res) => {
  try {
    ...
    const chatId = Object.prototype.hasOwnProperty.call(req.body || {}, 'chat_id') ? sanitizeChatId(req.body.chat_id) : existing.chat_id;
    const inviteUrl = Object.prototype.hasOwnProperty.call(req.body || {}, 'invite_url') ? sanitizeInviteUrl(req.body.invite_url) : existing.invite_url;
...
router.post('/actions/test-notification', async (req, res) => {
  try {
    const result = await enqueueOrganizerTestNotification(req.db);
    res.status(202).json({ success: true, ...result });
  } catch (error: any) {
    res.status(409).json({ error: error?.message || 'Не удалось поставить тестовое уведомление в очередь' });
  }
});

router.post('/actions/health', async (_req, res) => {
  try { res.json(await checkTelegramRuntimeHealth()); }
  catch (error: any) { res.status(500).json({ error: error?.message || 'Не удалось выполнить проверку Telegram' }); }
});
