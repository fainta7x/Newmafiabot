...
export const calculateClubGamePlayerTokens = (input: ClubGameTokenFormulaInput): ClubGameTokenBreakdown => {
  ...
  const rawDisciplinary = Number(input.disciplinaryPoints || 0);
  const legacySignedDisciplinary = Number.isFinite(rawDisciplinary) && rawDisciplinary < 0 ? rawDisciplinary : 0;
  const additionalPointsTenths = [input.judgeBonus,input.protocolBonus,input.bestMovePoints,input.ciPoints,legacySignedDisciplinary].map(decimalPointsToTenths).reduce((sum, value) => sum + value, 0);
  ...
  const uncappedTotal = participation + victory + additionalPointsTokens + foulBonus + minorTechnicalPenalty + majorTechnicalPenalty + removalPenalty + ppkPenalty;
  if (!Number.isSafeInteger(uncappedTotal)) throw new Error('Расчёт жетонов выходит за безопасный целочисленный диапазон');
  return { participation, victory, additional_points_tenths: additionalPointsTenths, additional_points_tokens: additionalPointsTokens, foul_bonus: foulBonus, minor_technical_penalty: minorTechnicalPenalty, major_technical_penalty: majorTechnicalPenalty, removal_penalty: removalPenalty, ppk_penalty: ppkPenalty, uncapped_total: uncappedTotal, total: Math.max(-1000, uncappedTotal) };
};
...
const bestMovePointsForParticipant = (protocol: any, participantId: string, playerResults: any[]): number => {
  ...
  const relevant = modern.filter((move: any) => move?.source === 'first_killed' && String(move?.participant_id || '') === participantId);
  if (relevant.length) return relevant.reduce((sum: number, move: any) => sum + calculateBestMovePoints(move?.seat_numbers, playerResults), 0);
  if (String(protocol?.best_move_participant_id || '') === participantId && (!protocol?.best_move_source || protocol.best_move_source === 'first_killed')) return calculateBestMovePoints(protocol?.best_move_seats, playerResults);
  return 0;
};
...
const buildDesiredTargets = async (db: DatabaseWrapper, game: any): Promise<Map<string, DesiredTarget>> => {
  ...
  const ppkParticipantId = payload.protocol?.end_reason === 'ppk' ? String(payload.protocol?.ppk_culprit_participant_id || '') : '';
  for (const result of linkedResults) {
    const playerId = String(result.player_id);
    const participantId = String(result.participant_id || '');
    const breakdown = calculateClubGamePlayerTokens({ role: result.role, winnerTeam, judgeBonus: result.judge_bonus, protocolBonus: result.protocol_bonus, bestMovePoints: bestMovePointsForParticipant(payload.protocol, participantId, results), ciPoints: result.ci_points, disciplinaryPoints: result.disciplinary_penalty_points, regularFouls: result.regular_fouls, minorTechnicalFouls: result.minor_technical_fouls, majorTechnicalFouls: result.major_technical_fouls, removed: result.exit_type === 'removed', ppkCulprit: Boolean(participantId && participantId === ppkParticipantId) });
...
export const reconcileClubGameTokenSettlement = async (db: DatabaseWrapper, gameId: number, options: ReconcileClubGameSettlementOptions) => {
  ...
  for (const key of [...keys].sort()) {
    ...
    if (delta !== 0) {
      ...
      const description = subjectType === 'judge' ? `Игра №${game.global_game_number}: ${actionWord} жетонов судье` : `Игра №${game.global_game_number}: ${actionWord} жетонов игроку`;
      await mutateTokenBalance(db, { playerId, delta, reasonType, description, sourceType: reasonType, sourceId: String(gameId), idempotencyKey: `club-game:${gameId}:${subjectType}:${playerId}:rev:${revision}`, debitPolicy: 'allow_negative', actorType: 'system', actorId: null, metadata: { game_id: gameId, game_number: Number(game.global_game_number), subject_type: subjectType, previous_target: previousAmount, target: nextAmount, delta, revision, context: options.context, breakdown } });
      mutations += 1;
    }
    await db.run(`INSERT INTO club_game_token_settlements (game_id, subject_type, player_id, target_amount, revision, breakdown_json, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(game_id, subject_type, player_id) DO UPDATE SET target_amount = excluded.target_amount, revision = excluded.revision, breakdown_json = excluded.breakdown_json, updated_at = excluded.updated_at`, [gameId, subjectType, playerId, nextAmount, revision, breakdown ? JSON.stringify(breakdown) : null, now]);
  }
