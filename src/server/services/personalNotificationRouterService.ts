...
export async function loadPersonalNotificationPreference(db: DatabaseWrapper, playerId: string) {
  ...
  const row = await db.get<any>(`
    SELECT preferred_channel, personal_enabled, updated_at
...
export async function savePersonalNotificationPreference(
  db: DatabaseWrapper,
  playerId: string,
  input: { preferredChannel?: unknown; personalEnabled?: unknown },
) {
  await ensurePersonalNotificationRoutingSchema(db);
  const current = await loadPersonalNotificationPreference(db, playerId);
  const preferredChannel = input.preferredChannel === undefined
    ? current.preferred_channel
    : normalizePreference(input.preferredChannel);
  const personalEnabled = input.personalEnabled === undefined
    ? current.personal_enabled
    : Boolean(input.personalEnabled);
  const now = nowIso();
  await db.run(`
...
export async function resolvePersonalNotificationRouting(
  db: DatabaseWrapper,
  playerId: string,
  ...
  const [preference, player, vkIdentity] = await Promise.all([
    ...
    db.get<any>(`
      SELECT external_user_id
       ...
       WHERE platform = 'vk' AND player_id = ?
       LIMIT 1
    `, [playerId]),
  ]);
...
export async function queuePersonalNotification(db: DatabaseWrapper, input: PersonalNotificationInput) {
  ...
  const existing = await db.get<any>(`
    SELECT * FROM personal_notification_deliveries WHERE notification_key = ? LIMIT 1
  `, [notificationKey]);
  ...
  await db.run(`
    INSERT INTO personal_notification_deliveries (
      notification_key, player_id, category, event_type, entity_id,
      selected_channel, channel_target, text, action_path,
  ...
  if (selectedChannel === 'telegram' && routing.channel_target) {
    await enqueueTelegramMessage(db, {
      messageKey: `personal:${notificationKey}:telegram`,
      category: 'personal',
      eventType,
