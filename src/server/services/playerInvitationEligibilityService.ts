...
export type InvitationContext = {
  ...
  evenings: Array<{
    ...
    venue: string | null;
    format: string;
    state: InvitationEveningState;
    existing_invitation: any | null;
  }>;
};
...
export async function loadInvitationContextsForRecipients(
  db: DatabaseWrapper,
  ...
  const recipients = await db.all<any>(`SELECT id,nickname,telegram_user_id,game_level,COALESCE(contact_status,lifecycle_status,'normal') AS status FROM players WHERE id IN (${placeholders(ids)})`, ids);
  ...
  const invitationRows = eveningIds.length ? await db.all<any>(`
    SELECT id,evening_id,recipient_player_id,status,created_at
      FROM player_evening_invitations
     WHERE inviter_player_id=? AND evening_id IN (${placeholders(eveningIds)}) AND recipient_player_id IN (${placeholders(ids)})
  `,[inviterPlayerId,...eveningIds,...ids]) : [];
  const invitationMap = new Map(invitationRows.map((row:any) => [`${row.evening_id}:${row.recipient_player_id}`,row]));
  const senderCounts = eveningIds.length ? await db.all<any>(`
    SELECT evening_id,COUNT(DISTINCT recipient_player_id) AS total
      FROM player_evening_invitations
     WHERE inviter_player_id=? AND evening_id IN (${placeholders(eveningIds)}) GROUP BY evening_id
  `,[inviterPlayerId,...eveningIds]) : [];
...
export async function createHardenedEveningInvitation(db: DatabaseWrapper, inviterPlayerId: string, recipientPlayerId: string, eveningId: string) {
  await ensurePremiumPlayerConnectionsSchema(db);
  const contexts = await loadInvitationContextsForRecipients(db,inviterPlayerId,[recipientPlayerId]);
  const context = contexts.get(recipientPlayerId);
  ...
  const [inviter,recipient] = await Promise.all([
    ...
    db.get<any>('SELECT telegram_user_id FROM players WHERE id=? LIMIT 1',[recipientPlayerId]),
  ]);
  const id=`evening_invite_${crypto.randomUUID()}`; const now=new Date().toISOString();
  ...
  if (recipient?.telegram_user_id) {
    ...
    const base=String(process.env.PLAYER_APP_URL||'').replace(/\/$/,'');
    const link=base?`${base}/player/events/${encodeURIComponent(eveningId)}`:'';
    const action=link?`\n\n<a href="${htmlEscape(link)}">Открыть этот вечер</a>`:'\n\nОткрой личный кабинет и перейди к этому вечеру.';
    await enqueueTelegramMessage(db,{messageKey:`evening-invite:${id}`,category:'personal',eventType:'evening_invite',entityId:id,playerId:recipientPlayerId,chatId:String(recipient.telegram_user_id),text:`🎲 <b>${htmlEscape(inviter?.nickname||'Игрок')}</b> зовёт тебя на «${htmlEscape(candidate.title)}» · ${htmlEscape(starts)}.${action}\n\nПриглашение не создаёт запись автоматически.`});
