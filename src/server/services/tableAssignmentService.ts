...
export async function assignParticipantToTable(
  ...
  _targetRegistrationStatus?: string | null,
  eveningIdParam?: string,
) {
  const participant = await db.get<any>('SELECT * FROM evening_participants WHERE id = ?', [participantId]);
  if (!participant) throw new TableAssignmentError('Участник не найден', 404);
  const evening = await db.get<any>('SELECT * FROM game_evenings WHERE id = ?', [participant.evening_id]);
  ...
  if (finalTableId) {
    const table = await db.get<any>('SELECT * FROM evening_tables WHERE id = ?', [finalTableId]);
  ...
  // 04B0: moving a participant between tables never mutates response or actual attendance.
