...
export async function reorderTournamentReserve(db:DatabaseWrapper,tournamentId:string,registrationIds:string[],reason:string,actorId?:string|null){
  ...
  return serializeTournamentMutation(db,()=>db.transaction(async(tx)=>{const tournament=await tx.get<any>('SELECT status FROM tournaments WHERE id=?',[tournamentId]);if(!tournament)throw new Error('TOURNAMENT_NOT_FOUND');if(tournament.status!=='draft')throw new Error('ROSTER_LOCKED');await renumberReserve(tx,tournamentId,registrationIds);await audit(tx,tournamentId,'reserve_reorder','organizer',actorId,null,reason,{registration_ids:registrationIds});return loadTournamentEvening(tx,tournamentId);}));
}
...
export async function promoteTournamentReserve(db:DatabaseWrapper,tournamentId:string,playerId:string,reason:string,actorId?:string|null){
  if(!reason.trim())throw new Error('REASON_REQUIRED');
  const promoted=await serializeTournamentMutation(db,()=>db.transaction(async(tx)=>{const tournament=await tx.get<any>('SELECT * FROM tournaments WHERE id=?',[tournamentId]);if(!tournament)throw new Error('TOURNAMENT_NOT_FOUND');if(tournament.status!=='draft')throw new Error('ROSTER_LOCKED');const row=await tx.get<any>("SELECT * FROM tournament_registrations WHERE tournament_id=? AND player_id=? AND status='reserve'",[tournamentId,playerId]);if(!row)throw new Error('NOT_IN_RESERVE');const slot=await nextFreeSlot(tx,tournamentId);if(slot==null)throw new Error('TOURNAMENT_FULL');const now=nowIso();await tx.run("UPDATE tournament_registrations SET status='confirmed',slot_number=?,queue_order=NULL,updated_at=?,organizer_reason=? WHERE id=?",[slot,now,reason,row.id]);await renumberReserve(tx,tournamentId);await syncCanonicalParticipants(tx,tournamentId);await audit(tx,tournamentId,'manual_promote','organizer',actorId,playerId,reason,{slot_number:slot});return {playerId,slot};}));
