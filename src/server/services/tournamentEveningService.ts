...
export async function promoteTournamentReserve(db:DatabaseWrapper,tournamentId:string,playerId:string,reason:string,actorId?:string|null){
  ...
  await queuePersonalNotification(db,{notificationKey:`tournament:${tournamentId}:manual-promotion:${playerId}`,playerId,eventType:'tournament_reserve_promoted',entityId:tournamentId,text:'Организатор перевёл вас из резерва в основной состав турнира.',actionPath:`/player?tournament=${encodeURIComponent(tournamentId)}`});
  return promoted;
}
