...
const saveCredential = async (db: DatabaseWrapper, payload: VkTokenPayload, deviceId: string, previous?: OAuthCredentialRow | null) => {
  ...
  if (!accessToken) throw new Error('VK ID не вернул access_token');
  const refreshToken = String(payload.refresh_token || previous?.refresh_token || '').trim() || null;
  const scope = String(payload.scope || previous?.scope || VK_SCOPES.join(' ')).trim();
  ...
  return { user_id: userId, scope, expires_at: expiresAt };
};
...
export async function createVkOAuthStart(db: DatabaseWrapper, input: { redirectUri: string; returnTo?: unknown }) {
  ...
  await db.run(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `, [state, verifier, redirectUri, returnTo, now.toISOString(), expiresAt]);
...
export async function completeVkOAuth(db: DatabaseWrapper, input: {
  code: unknown;
  deviceId: unknown;
  state: unknown;
}) {
  ...
  const pending = await db.get<OAuthStateRow>(`
    SELECT state, verifier, redirect_uri, return_to, expires_at
...
export async function hydrateVkOAuthAccessToken(db: DatabaseWrapper): Promise<boolean> {
  ...
  if (expiresAt > Date.now() + REFRESH_EARLY_MS) {
    setVkRuntimeUserToken(credential.access_token);
    return true;
  }

  try {
    if (await refreshCredential(db, credential)) return true;
  } catch (error) {
    console.error('[VK OAUTH] token refresh failed:', error);
  }
