...
describe('Bogdana historical correction migration', () => {
  it('applies once and records a durable completion marker', async () => {
    ...
    await db.run(
      `INSERT INTO tournaments (id, title, date, created_at)
       VALUES ('t1', 'Турнир Богдана 1.08', '2026-08-01', '2026-08-01T00:00:00.000Z')`,
    );
    ...
    await db.run(
      `INSERT INTO tournament_participants (id, tournament_id, player_id, display_name)
       VALUES ('tp1', 't1', 'p1', 'Богданчик')`,
    );
    await db.run(
      `INSERT INTO tournament_game_player_results (id, game_id, participant_id, judge_bonus)
       VALUES ('r1', 'g10', 'tp1', 0.4)`,
    );
    ...
    const result = await db.get<any>(
      `SELECT judge_bonus FROM tournament_game_player_results WHERE id='r1'`,
    );
    expect(Number(result?.judge_bonus)).toBeCloseTo(0.7);

    const markers = await db.all<any>(
      `SELECT migration_name, status, details_json
