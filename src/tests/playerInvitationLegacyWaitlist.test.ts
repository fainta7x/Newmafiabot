...
describe('player invitation legacy waitlist normalization', () => {
  ...
  beforeEach(async () => {
    ...
    await db.run(`
      ...
      VALUES
        ...
        ('legacy-late','Legacy late','club','normal','normal','none',1000,100,?,?)
    ...
    await db.run(`
      ...
      VALUES
        ...
        ('p-late','evening','legacy-late','unanswered','waitlist','pending','late','unpaid',0,0,?,?)
    `,[now,now,now,now,now,now]);
  });
  ...
  it('honors canonical late normalization when historical arrival proves a late response', async () => {
    const contexts = await loadInvitationContextsForRecipients(db,'inviter',['legacy-late']);
    const context = contexts.get('legacy-late');
    expect(context?.can_invite).toBe(false);
    expect(context?.evenings[0]?.state).toBe('registered');
  });
});
