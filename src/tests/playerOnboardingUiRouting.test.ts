...
describe('VK-ACCESS-004 shared onboarding UI routing', () => {
  it('moves new Telegram identities onto the shared verified onboarding flow', () => {
    ...
    expect(app).not.toContain("fetch('/api/auth/register'");
    expect(auth).toContain('beginVerifiedPlayerOnboarding');
    expect(auth).toContain("platform: 'telegram'");
  ...
  it('verifies VK before requesting any nickname', () => {
    ...
    expect(start).toContain('VERIFIED_ONBOARDING_PLACEHOLDER');
    expect(callback).toContain("platform: 'vk'");
    expect(callback).toContain('beginVerifiedPlayerOnboarding');
    expect(callback).toContain('setPlayerOnboardingCookie(res, onboarding.token)');
  });
  ...
  it('keeps the raw onboarding token in an HttpOnly cookie and exposes only safe status to React', () => {
    ...
    expect(auth).toContain("router.get('/onboarding'");
    expect(auth).not.toContain('external_user_id: pending');
    expect(ui).toContain("fetch('/api/auth/onboarding'");
    expect(ui).not.toContain('externalUserId');
    expect(ui).not.toContain('player_id');
  });
  ...
  it('uses one channel-neutral choice and completion API for both Telegram and VK', () => {
    ...
    expect(auth).toContain("router.post('/onboarding/new'");
  ...
  it('does not change the legacy public VK evening-registration branch', () => {
    const callback = read('src/server/services/vkJoinRegistrationCallbackRouter.ts');
    const boundary = callback.indexOf('// Existing public evening-registration VK flow remains unchanged.');
    expect(boundary).toBeGreaterThan(0);
    const publicBranch = callback.slice(boundary);
    expect(publicBranch).toContain('peekVkJoinOAuthState');
