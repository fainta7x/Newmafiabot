...
import { drainVkMessageOutbox, enqueueVkMessage, sendVkCommunityMessage, startVkMessageOutboxWorker } from '../server/services/vkMessageOutboxService.ts';
...
describe('VK personal messages.send delivery',()=>{
  it('holds cancellation DMs while the emergency pause is active', async () => {
    vi.stubEnv('WEEKLY_EVENING_AUTOMATION_ENABLED', 'false');
    const db = createDatabaseConnection(':memory:');
    try {
      await enqueueVkMessage(db, { messageKey: 'personal:cancel:vk', notificationKey: 'cancel', category: 'personal', eventType: 'evening_cancelled', vkUserId: '123', text: 'Отмена' });
      const send = vi.fn();
      expect(await drainVkMessageOutbox(db, { fetchImpl: send as typeof fetch })).toMatchObject({ processed: 0 });
      expect(send).not.toHaveBeenCalled();
      expect(await db.get<any>("SELECT status FROM vk_message_outbox WHERE message_key='personal:cancel:vk'")).toMatchObject({ status: 'pending' });
